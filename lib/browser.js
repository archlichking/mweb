...
internals.Browser = function(config) {
    ...
    this.timeout = config.timeout ? config.timeout : 80000;
    this._server = new SeleniumServer(seleniumServerJar.path, {
        port: 4444
    });

    this._capacities = config.getOriginalConfig().browser;
    ...
    switch (config.browser.name.toLowerCase()) {
        ...
        default:
            ...
            throw new Error('\t- Browser: ' + config.browser.name + ' is not supported...');
            break;
    }
